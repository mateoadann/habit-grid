...
describe("GET /api/habits/:id", () => {
  ...
  it("returns 404 for non-existent habit", async () => {
    const res = await request(app).get("/api/habits/nonexistent_id");
...
describe("PUT /api/habits/:id", () => {
  it("updates an existing habit", async () => {
    ...
    const res = await request(app).put(`/api/habits/${habitId}`).send({
      name: "Read Books",
      emoji: "📚",
  ...
  it("returns 404 for non-existent habit", async () => {
    const res = await request(app).put("/api/habits/nonexistent_id").send({
      name: "Nope",
      unit_id: 1,
      minimum: 1,
    });
...
describe("DELETE /api/habits/:id", () => {
  it("deletes an existing habit", async () => {
    const createRes = await request(app).post("/api/habits").send({
      name: "To Delete",
      unit_id: 1,
      minimum: 1,
    });
    ...
    const getRes = await request(app).get(`/api/habits/${habitId}`);
    expect(getRes.status).toBe(404);
  });

  it("returns 404 for non-existent habit", async () => {
    const res = await request(app).delete("/api/habits/nonexistent_id");
